...
function createPostseasonBracket() {
	if (!hasFullAppAccess()) {
		alert("Open the main menu to create the postseason bracket.");
		return;
	}
...
function resetPostseason() {
	if (!hasFullAppAccess()) {
		alert("Open the main menu to reset postseason.");
		return;
	}
...
async function startPostseasonGame(slotId) {
	if (!hasFullAppAccess()) {
		alert("Open the main menu to record postseason games.");
		return;
	}
...
function displayManualGameStatEditor(selectedGameId = "") {
	...
	if (isPublicViewOnlyMode() || !unlocked) {
		container.innerHTML = `
			<div class="card">
				<p class="season-stats-empty">Open the main menu to edit completed game stats.</p>
			</div>
		`;
		return;
	}
