...
async function beginLockedGame(t1,t2,scheduleRef=null,extraLockDetails={},gameContext=null) {
  return startRecordingGame(t1,t2,scheduleRef,extraLockDetails,gameContext);
}
...
function runLiveGameAction(actionLabel,fn) { return recordAction(actionLabel,fn); }


async function runGameStartAction(fn) {
  if(gameStartInProgress || recording.busy) return false;
  gameStartInProgress=true;
  try { return await fn(); }
  catch(error) { alert(error.message); return false; }
  finally { gameStartInProgress=false; renderRecordingStatus(); }
}
...
async function endGameEarly() {
  if (!recordingCanAct()) return alert('Recording is paused. Reconnect or resume ownership first.');
...
async function emergencyEndGameFromSetup() { showMainMenu(); }
