...
function applyPitcherSelectionLockState() {
	if (game && (game._finalizeInProgress || game._gameCompletePendingSave)) { renderRecordingStatus(); return; }
...
function updatePitcherDisplay(options = {}) {
  if(!options.renderOnly && !recording.actionRunning) {
    return recordAction('pitcher',()=>updatePitcherDisplay(options),{skipPitcher:true});
  }
	let select = document.getElementById("pitcherSelect");
	if (!select || !game?.fielding?.players) return;
