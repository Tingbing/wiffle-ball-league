...
// Split from app.core.js. Load this AFTER core.schedule.js and BEFORE core.ui.js, app.game.js, and app.boot.js.

/* ================================
   PERSISTED SEASON / SCHEDULE SNAPSHOTS
================================== */
function saveSchedule() { return true; }
function loadSchedule() { schedule=ensureScheduleShape(readJsonStorage(SCHEDULE_STORAGE_KEY,schedule)); }
...
function saveSeason() { return true; }
function loadSeason() { season=ensureSeasonShape(readJsonStorage(SEASON_STORAGE_KEY,season)); }
...
function applyGameOutcomeOnce() {
  ...
  markCompletedGameOutcomeApplied(getCompletedGameEntryId(game));
...
function persistRestoredSeasonAndSchedule(nextSeason,nextSchedule) {
  season=ensureSeasonShape(deepCloneJson(nextSeason));
  schedule=ensureScheduleShape(deepCloneJson(nextSchedule));
  return true; // The enclosing league edit writes both snapshots in one server transaction.
}

function downloadStatsBackupJson() {
	if (isPublicViewOnlyMode()) {
		alert("Open the main menu before downloading a stats backup.");
		return;
	}
...
function warnIfDestructiveActionUnsafe(actionLabel) {
	...
	alert(
		...
		"Resume the in-progress game from the main menu and finish it first."
	);
	return true;
}
...
async function clearCurrentStatsOnly({ skipConfirm = false, quiet = false, syncToServer = true } = {}) {
		if (isPublicViewOnlyMode()) {
		alert("Open the main menu before clearing season stats.");
		return false;
	}
...
function openStatsRestorePicker() {
	if (isPublicViewOnlyMode()) {
		alert("Open the main menu before restoring a stats backup.");
		return;
	}
...
async function resetSeason() {
  if(warnIfDestructiveActionUnsafe('resetting the season')) return false;
  if(!confirm('Reset all season stats, completed games and schedule results? Download a backup first.')) return false;
  return await clearCurrentStatsOnly({skipConfirm:true,quiet:true,syncToServer:false});
}
